...
**Issue relacionada:** [#7 — SDD-002 — GitHub Flow](https://github.com/AlexBiudes/projetopilotoBI/issues/7)
**Branch obrigatória:** `feat/github-flow`
**Diretório proprietário:** `src/features/github-flow/`
...
Esta SPEC cobre a frente de Alex: um módulo educacional sobre o fluxo colaborativo com GitHub, desenvolvido em paralelo com a SDD-001 (Fundamentos de Git, responsabilidade de Diego). O objetivo do roadmap para esta rodada é praticar trabalho simultâneo sem colisão de escopo — por isso o isolamento de diretórios descrito na seção 6 desta SPEC é uma exigência dura, não uma sugestão.

A estrutura de diretórios `src/features/github-flow/` já existe no repositório (contendo apenas `.gitkeep`), preparada pela SDD-000 para receber esta feature.

---

## 2. Objetivo

Criar um módulo educacional claro, visual e interativo sobre o fluxo colaborativo com GitHub — o processo que o próprio projeto pratica (ROADMAP, seção 2, "o próprio projeto deve praticar o processo que ensina").

Ao final, o módulo deve permitir que um iniciante compreenda o ciclo completo `Issue → SPEC → Branch → Implementação → Commit → Push → Pull Request → Review → Correção → Approve → Merge → Atualizar main`.

---

## 3. Responsável humano

**Alex** — autor da feature e responsável por validar o comportamento funcional antes da revisão cruzada. Alex não deve aprovar a própria implementação apenas por ter sido executada por seu agente (ROADMAP, seção 4.2).

---

## 4. Issue relacionada

[#7 — SDD-002 — GitHub Flow](https://github.com/AlexBiudes/projetopilotoBI/issues/7)
...
O agente futuro pode, **majoritariamente dentro de `src/features/github-flow/`**:
...
```text
src/features/github-flow/
├── GithubFlowPage.tsx          (ou nome equivalente — componente de entrada do módulo)
...
4. funcionar de forma independente do módulo Fundamentos de Git (SDD-001) — nenhuma dependência cruzada de componentes ou estado entre as duas features.
...
2. o fluxo visual obrigatório (seção 10) deve ser apresentado como um diagrama sequencial claro (JSX/CSS simples, não é necessário biblioteca de diagramação);
3. distinção visual clara entre os três tipos de review (Comment, Approve, Request Changes), por exemplo com cores ou ícones diferentes;
...
- merge;
- delete branch;
- sincronização da main após merge.

Fluxo visual obrigatório:

```
Issue → SPEC → Branch → Implementação → Commit → Push → Pull Request → Review → Correção → Approve → Merge → Atualizar main
```

A feature deve explicar:

- por que não desenvolver diretamente na main;
- por que cada feature usa uma branch;
- por que PR existe;
- diferença entre Comment, Approve e Request Changes;
- o que acontece quando um PR é atualizado;
- o que significa merge;
- o que significa squash;
- por que deletar a branch depois do merge;
- como outro desenvolvedor recebe a mudança.
...
- [ ] fluxo visual completo (`Issue → SPEC → Branch → ... → Atualizar main`) implementado;
- [ ] diferença entre Comment, Approve e Request Changes explicada de forma clara;
- [ ] nenhuma alteração em `src/features/fundamentos-git/`;
...
- fazer deploy;
- alterar secrets;
- mexer em `src/features/fundamentos-git/`;
...
O agente **pode**: sincronizar `main`, criar a branch `feat/github-flow`, implementar, testar, commitar, fazer push e abrir PR.

---

## 15. Risco de colisão

Diretório proprietário desta SPEC: `src/features/github-flow/`.

Diretório proprietário da SDD-001 (Diego): `src/features/fundamentos-git/`.
...
3. reportada com o nome do arquivo e o risco de colisão com a SDD-001, que provavelmente precisará da mesma alteração para sua própria aba;
...
1. a branch `feat/github-flow` deve partir da `main` atualizada (pós-merge da fundação SDD-000);
2. o PR desta feature deve ser aberto contra `main`, referenciando a Issue #7 e esta SPEC;
3. revisão cruzada: Diego revisa a implementação de Alex (ROADMAP, seção 10);
4. se houver conflito com a branch `feat/fundamentos-git` (SDD-001) em arquivos compartilhados, a resolução deve ser coordenada pelo supervisor técnico, não decidida unilateralmente pelo agente;
...
9. confirmação explícita de que não houve merge, deploy, alteração de secrets, comandos destrutivos, nem alterações em `src/features/fundamentos-git/`.

---

## 18. Próxima fase

Após o merge desta feature (e da SDD-001), a Fase 2 do roadmap (Branches, Commits, Pull Requests, Code Review) pode ser iniciada, conforme [ROADMAP_DESENVOLVIMENTO_EQUIPE_IA_SDD.md](../ROADMAP_DESENVOLVIMENTO_EQUIPE_IA_SDD.md), seção 11.
