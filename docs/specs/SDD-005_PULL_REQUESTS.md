...
**Issue relacionada:** [#13 — SDD-005 — Pull Requests](https://github.com/AlexBiudes/projetopilotoBI/issues/13)
**Branch obrigatória:** `feat/pull-requests`
**Diretório proprietário:** `src/features/pull-requests/`

---

## 1. Contexto

O projeto está na Fase 2 do roadmap (ver [ROADMAP_DESENVOLVIMENTO_EQUIPE_IA_SDD.md](../ROADMAP_DESENVOLVIMENTO_EQUIPE_IA_SDD.md), seção 11, Fase 2). Esta SPEC cobre a Rodada 2B, frente de Diego, e só deve ser executada após o encerramento controlado da Rodada 2A (SDD-003 — Branches, e SDD-004 — Commits, ambas mergeadas na `main`), salvo nova autorização explícita do supervisor técnico.

Esta SPEC cobre um módulo educacional sobre Pull Requests, desenvolvido em paralelo com a SDD-006 (Code Review, responsabilidade de Alex). O isolamento de diretórios descrito na seção 6 é uma exigência dura, repetindo o padrão validado nas rodadas anteriores.
...
Criar um módulo educacional claro, visual e interativo sobre Pull Requests — o mecanismo pelo qual mudanças em uma branch são propostas, revisadas e integradas à `main`.

Ao final, o módulo deve permitir que um iniciante compreenda o ciclo completo de um PR, desde a abertura até o merge, incluindo revisão, correções e diferentes estratégias de merge.
...
[#13 — SDD-005 — Pull Requests](https://github.com/AlexBiudes/projetopilotoBI/issues/13)
...
O agente futuro pode, majoritariamente dentro de `src/features/pull-requests/`:
...
- qualquer alteração em `src/features/code-review/` (diretório proprietário da SDD-006, Alex);
...
- implementação de conteúdo ou funcionalidades da SDD-006;
...
```text
src/features/pull-requests/
├── PullRequestsPage.tsx      (ou nome equivalente — componente de entrada do módulo)
...
- Draft PR;
- Ready for Review;
- atualização do mesmo PR após novos pushes;
- conflitos;
- mergeability;
- merge;
- squash merge;
- merge commit;
- rebase merge (apenas como conceito, sem exigir prática);
- fechamento sem merge;
- exclusão da branch após merge;
- por que não criar novo PR para cada correção;
- exercício ou simulação prática.

Fluxo visual obrigatório:

```
Issue → SPEC → Branch → Implementação → Commit → Push → Pull Request → Review → Correção → Novo push no mesmo PR → Approve → Merge
```
...
- [ ] fluxo visual completo (`Issue → SPEC → ... → Merge`) implementado;
- [ ] diferença entre squash merge, merge commit e rebase merge (conceitual) explicada;
- [ ] exercício/simulação prática do ciclo de vida de um PR presente;
- [ ] nenhuma alteração em `src/features/code-review/`, `src/features/branches/`, `src/features/commits/`, `src/features/fundamentos-git/` ou `src/features/github-flow/`;
...
- fazer deploy;
- alterar secrets;
- mexer em `src/features/code-review/`, `src/features/branches/`, `src/features/commits/`, `src/features/fundamentos-git/` ou `src/features/github-flow/`;
...
O agente **pode**: sincronizar `main`, criar a branch `feat/pull-requests`, implementar, testar, commitar, fazer push e abrir PR.

---

## 16. Risco de colisão

Diretório proprietário desta SPEC: `src/features/pull-requests/`.

Diretório proprietário da SDD-006 (Alex, mesma rodada): `src/features/code-review/`.
...
3. deve ser reportada com o nome do arquivo e o risco de colisão com a SDD-006, que também precisará da mesma alteração para sua própria aba;
4. não deve incluir reformatação ou refatoração do restante do arquivo.

Se esta feature for a primeira das duas a ser mergeada na Rodada 2B, a branch `feat/code-review` (SDD-006) precisará sincronizar com a `main` pós-merge antes de finalizar seu próprio PR (ver seção 17).

---

## 17. Estratégia de integração

1. a branch `feat/pull-requests` deve partir da `main` atualizada (pós-merge de SDD-003 e SDD-004);
2. o PR desta feature deve ser aberto contra `main`, referenciando a Issue #13 e esta SPEC;
...
6. **smoke local pós-integração:** após a segunda branch sincronizar com a `main`, repetir `npm run lint`, `npm run build`, `git diff --check` e o smoke visual, confirmando que ambas as abas (Pull Requests e Code Review) continuam funcionando;
...
8. riscos e pendências, incluindo status da sincronização com `feat/code-review` caso o merge sequencial já tenha ocorrido;
9. confirmação explícita de que não houve merge, deploy, alteração de secrets, comandos destrutivos, nem alterações em `src/features/code-review/`, `src/features/branches/`, `src/features/commits/`, `src/features/fundamentos-git/` ou `src/features/github-flow/`.

---

## 19. Próxima fase

Após o merge desta feature (e da SDD-006), a Fase 3 do roadmap (Primeiro conflito controlado) pode ser iniciada, mediante nova autorização explícita, conforme [ROADMAP_DESENVOLVIMENTO_EQUIPE_IA_SDD.md](../ROADMAP_DESENVOLVIMENTO_EQUIPE_IA_SDD.md), seção 11, Fase 3.
